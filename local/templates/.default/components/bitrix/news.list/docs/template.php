...
{
	return $matches[0]{
	0} . '<span style="background-color: #FFFF77; font-weight: bolder;">' . substr($matches[0], 1) . '</span>';
}
...
<div class="news-list1">
	...
	?>
		...
		<div class="news-items-wrapper">
			...
			<div class="news-item" id="<?= $this->GetEditAreaId($arItem['ID']); ?>" style="display: inline-block; vertical-align: middle;">
				<? if ($arParams["DISPLAY_PICTURE"] != "N" && is_array($arItem["PREVIEW_PICTURE"])) : ?>
					<? if (!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])) : ?>
						<a href="<?= $arItem["DETAIL_PAGE_URL"] ?>&q=<?= $_GET['q'] ?>"><img class="preview_picture" border="0" src="<?= $arItem["PREVIEW_PICTURE"]["SRC"] ?>" width="<?= $arItem["PREVIEW_PICTURE"]["WIDTH"] ?>" height="<?= $arItem["PREVIEW_PICTURE"]["HEIGHT"] ?>" alt="<?= $arItem["PREVIEW_PICTURE"]["ALT"] ?>" title="<?= $arItem["PREVIEW_PICTURE"]["TITLE"] ?>" style="float:left" /></a>
					<? else : ?>
